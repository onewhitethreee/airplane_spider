...
class TripScraper(FlightScraper):
    ...
    def __init__(self, platform_config):
        ...
        self._save_file_path = (
            f"flights_{uuid.uuid4().hex}.json"  # 修复多日期搜索时文件名冲突的问题
        )
        self._platform_type = "trip"
    ...
    def load_data(self) -> bool:
        ...
        processor = DataProcessorFactory.create_processor(
            self._platform_type, self._raw_data
        )
    ...
    def format_result(self):
        ...
        try:
            ...
            for index, flight in enumerate(self._processed_offers):
                ...
                if price_info:
                    all_info.append(
                        f"价格: {price_info['total']} {price_info['currency']}"
                    )
                ...
                # 去程信息
                out_dep_time = flight.outbound.time_info["departure_time"].replace(
                    "T", " "
                )
                out_arr_time = flight.outbound.time_info["arrival_time"].replace(
                    "T", " "
                )
                ...
                all_info.append(
                    f"飞行时间: {flight.outbound.time_info['total_time_formatted']}"
                )
                ...
                if flight.outbound.transit:
                    ...
                    for i, airport in enumerate(flight.outbound.transit):
                        layover_time = flight.outbound.time_info["layovers"][
                            i
                        ].layover_time_formatted
                        transit_info.append(f"{airport} (停留 {layover_time})")
                    all_info.append(f"中转: {' → '.join(transit_info)}")

                # 返程信息
                in_dep_time = flight.inbound.time_info["departure_time"].replace(
                    "T", " "
                )
                ...
                all_info.append(
                    f"飞行时间: {flight.inbound.time_info['total_time_formatted']}"
                )
                ...
                if flight.inbound.transit:
                    ...
                    for i, airport in enumerate(flight.inbound.transit):
                        layover_time = flight.inbound.time_info["layovers"][
                            i
                        ].layover_time_formatted
                        transit_info.append(f"{airport} (停留 {layover_time})")
                    all_info.append(f"中转: {' → '.join(transit_info)}")
...
if __name__ == "__main__":
    ...
    scraper_config = ScraperFactory.create_scraper("trip")
